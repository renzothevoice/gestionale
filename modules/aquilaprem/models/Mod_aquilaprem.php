...
class Mod_Aquilaprem extends CI_Model
{
  ...
  /** estraggo i ToDo **/
  public function OLD_estrai_todo($cliente="0",$stato="0"){
    //$this->db->select('numero, data_creazione, rag_soc');
    $this->db->select('todo.*');
    $this->db->select('clienti.rag_soc');
    $this->db->from('todo');
    $this->db->join('clienti', 'clienti.id = todo.id_cliente');
  ...
  /** estraggo i Task **/
  public function OLD_estrai_tasks($cliente="0",$stato="0"){
    //$this->db->select('numero, data_creazione, rag_soc');
    $this->db->select('tasks.*');
    $this->db->select('tasks_categorie.*');
    $this->db->from('tasks');
    $this->db->join('tasks_categorie', 'tasks_categorie.id = tasks.id_categoria');
    //$this->db->where('id_azienda', $id_azienda);
    if($stato==0){
    $this->db->where('stato !=', 'CHIUSO');
    }
  ...
  /** elenco le scadenze **/
  public function OLD_elenca_scadenze($parametri){
    //$this->db->select('numero, data_creazione, rag_soc');
    $this->db->select('tasks.*');
    $this->db->select('tasks_categorie.*');
    $this->db->from('tasks');
